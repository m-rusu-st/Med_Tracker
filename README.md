...
If you need to install, please refer to [official Gradle docs](https://docs.gradle.org/current/userguide/installation.html).
...
Behind the scenes, the app actually saved the users in the database, encrypting the password. To see that it actually worked, we need to inspect the database that was created in the
...
Download the jar and run `java --module-path $PATH_TO_FX --add-modules javafx.controls,javafx.fxml $PATH_TO_NITRITE\nitrite-explorer-3.4.3.jar
